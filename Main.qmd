...
-   Encuestas de Stack Overflow (33880 registros obtenidos de 73268)
    ...
    library(dplyr)
    library(tableHTML)
    datos = encuesta[encuesta$MainBranch != "None of these" & 
                       complete.cases(encuesta$MainBranch) &
                       complete.cases(encuesta$Employment) &
                       complete.cases(encuesta$RemoteWork) &
                       complete.cases(encuesta$CodingActivities) &
                       complete.cases(encuesta$DevType) &
                       complete.cases(encuesta$Country) &
                       complete.cases(encuesta$EdLevel) &
                       complete.cases(encuesta$Currency) &
                       complete.cases(encuesta$ConvertedCompYearly) &
                       complete.cases(encuesta$MentalHealth) &
                       complete.cases(encuesta$Ethnicity) &
                       complete.cases(encuesta$Sexuality) &
                       complete.cases(encuesta$Gender) &
                       complete.cases(encuesta$Age) &
                       complete.cases(encuesta$`OpSysPersonal use`) & 
                       complete.cases(encuesta$`OpSysProfessional use`) &
                       complete.cases(encuesta$LanguageHaveWorkedWith),]

    datos = select(datos, MainBranch, Employment, RemoteWork, CodingActivities, DevType, Country, EdLevel, Currency, ConvertedCompYearly, MentalHealth, Ethnicity, Sexuality, Gender, Age, `OpSysPersonal use`, `OpSysProfessional use`, LanguageHaveWorkedWith)


    frec <- table(datos$Gender)
    frec <- sort(frec, decreasing = FALSE)
